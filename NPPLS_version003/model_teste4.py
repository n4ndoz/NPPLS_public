...
from GeometricUnit import GeomUnit
import os
...
class AnglePredictor(Model):
    def __init__(self, core_model='bilstm', ang_mode='alphabet',task='dih', **kwargs):
        '''
        ...
        self.conv1 = TCN(nb_filters=128, kernel_size=1, nb_stacks=4, dilations=[1, 2, 4, 8, 16,32],
        padding='causal', use_skip_connections=True, dropout_rate=0.2, return_sequences=True,
        activation='relu', kernel_initializer='he_normal', use_batch_norm=True)
        
        self.conv2 = TCN(nb_filters=64, kernel_size=1, nb_stacks=4, dilations=[1, 2, 4, 8, 16],
        padding='causal', use_skip_connections=True, dropout_rate=0.2, return_sequences=True,
        activation='elu', kernel_initializer='he_normal', use_batch_norm=True)
        '''
        self.conv3 = TCN(nb_filters=32, kernel_size=2, nb_stacks=2, dilations=[1, 2, 4, 8],
        padding='causal', use_skip_connections=True, dropout_rate=0.2, return_sequences=True,
        activation='elu', kernel_initializer='he_normal', use_batch_norm=True)
        '''
        self.alphabet = Alphabet(n_alphas=20, n_angles=3)
