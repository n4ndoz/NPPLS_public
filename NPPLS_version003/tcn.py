...
class TCN(Layer):
    ...
    def compute_mask(self, inputs, mask):
        # won't mess with the mask
        # Just return it as you received it
        return mask
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        '''
        From here on these are atempts to implement a gambiarra masking support.
        This will work like this:
        1- Receiving a masked tensor from a Masking layer (receiving a 'mask');
        2- Processing the input with all the TCN magic;
        3- Casting the 'mask' bool tensor to float32 and then
        4- The product of the last TCN output by the Float Mask.

        The ideal solution here would be to implement something with tf.boolean_mask
        and then pad the resulting tensor. 
        '''
        if self.return_sequences and mask is not None:
            # casts the mask to a float32 tensor
