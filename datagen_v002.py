...
from angle_utils_v4 import Dihedral_Calculator
...
class Batch:
    ...
    def print_stats(self):
        # this method describes a few statistics about the
        # current batch:
        # 1- Seq length distribution
        # 2- IDs
        # 3- Some plots
        # 3.1- 1
        # 3.2- Angle distribution (Ramachandran Space)
        # 3.3- Mean Angle
        # 3.4- etc
        # Won't mess with it for now, will left this for the last part of dev
        return 0
...
class Data_Prep_Pipeline:
    ...
    def parse_data_block(self, data_limit=None,
        ...
        desc_dict = utils.load_obj('desc_dict')
        ...
        pbar.close()
        return X, dihs, ids # X, y 

    def make_batch(self, data_limit = None, pad_to_cutoff=True, at_once=False, chunk_size=100, to_file=True):
    ...
    def make_batch(self,chunk_size=100, to_file=True, pad_to_cutoff=True,dims=[0,500,46],file_name='latest'):
        ...
        self.batch_num = 1
        if not pad_to_cutoff:
            X = []
            Y = []
            IDs = []
        else:
            X = np.empty(dims)
            Y = np.empty([0,dims[1],2])
            IDs= []
        
        while True:
            x, y, ids= self.parse_data_block(chunk_size)
            if len(x) <= 0:
        ...
        if to_file:
            print('to_file')
            utils.save_obj(X,'X_{}'.format(file_name),extension='.npy')
            utils.save_obj(Y,'Y_{}'.format(file_name),extension='.npy')
            utils.save_obj(ids,'IDs_{}'.format(file_name),extension='.pkl')
            return 1
        else:
            return X, Y, IDs
